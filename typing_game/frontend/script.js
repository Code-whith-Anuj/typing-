class TypingGame {
    ...
    initializeElements() {
        ...
        // Move Font Control to top of Stats Panel
        const statsPanel = document.querySelector('.stats-panel');
        const fontControl = document.querySelector('.font-control');
        if (statsPanel && fontControl) {
            statsPanel.insertBefore(fontControl, statsPanel.firstChild);
        }
    }
    ...
    bindEvents() {
        ...
        if (this.textSizeSlider) {
            this.textSizeSlider.addEventListener('input', (e) => this.setFontSize(e.target.value));
        }
    ...
    loadPreferences() {
        const savedSize = localStorage.getItem('typing_font_size');
        if (savedSize) {
            this.setFontSize(savedSize);
            if (this.textSizeSlider) this.textSizeSlider.value = savedSize;
        }
    }

    setFontSize(size) {
        document.documentElement.style.setProperty('--typing-font-size', `${size}px`);
        localStorage.setItem('typing_font_size', size);
    }

    handleKeyDown(e) {
        console.log("KEY PRESSED:", e.key, "gameReady:", this.gameActive);
    ...
    updateGameState(result, expectedChar, inputChar) {
        // Always sync error count from backend to ensure accuracy
        if (result.errors !== undefined) this.stats.errors = result.errors;
        ...
        if (result.correct) {
            ...
            this.stats.streak = 0;
            this.stats.combo = result.combo_multiplier;
            this.lastIncorrectKey = inputChar;
            
            // Visual feedback for incorrect key
            this.showFeedback('incorrect', inputChar);
    ...
    showFeedback(type, char) {
        const isLoveMode = document.body.classList.contains('love-mode');
        ...
        if (type === 'correct') {
            feedback.textContent = isLoveMode ? '❤' : '✓';
        } else {
            feedback.textContent = (char === ' ' ? '␣' : char);
        }
        
        const animation = type === 'correct' ? 'floatUp 0.8s ease-out forwards' : 'shake 0.5s ease-in-out forwards';
        
        const color = type === 'correct' ? (isLoveMode ? '#d81b60' : '#10b981') : '#ef4444';
    ...
    renderText() {
        console.log("RENDER TEXT CALLED WITH:", this.currentText);
        ...
        for (let i = 0; i < this.currentText.length; i++) {
            ...
            // Handle incorrect key display at current position
            if (i === this.currentPosition && this.lastIncorrectKey) {
                span.textContent = this.lastIncorrectKey === ' ' ? '·' : this.lastIncorrectKey;
                span.classList.add('incorrect', 'current');
            } else {
                // Normal display
                if (char === ' ') {
                    span.textContent = '·';
    ...
    updateStatsDisplay() {
        if (this.scoreEl) this.scoreEl.textContent = this.stats.score.toLocaleString();
        if (this.wpmEl) this.wpmEl.textContent = this.stats.wpm;
        if (this.accuracyEl) this.accuracyEl.textContent = `${this.stats.accuracy}%`;
        if (this.streakEl) this.streakEl.textContent = this.stats.streak;
        if (this.comboEl) this.comboEl.textContent = `${this.stats.combo.toFixed(1)}x`;
        if (this.errorsEl) this.errorsEl.textContent = this.stats.errors;
        if (this.maxStreakEl) this.maxStreakEl.textContent = this.stats.maxStreak;
    ...
    async startNewSession() {
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (!data.text) {
                    console.error("NO TEXT RECEIVED FROM BACKEND");
                    return;
                }

                this.sessionId = data.session_id;
                this.currentText = data.text;
                console.log("TEXT FROM BACKEND:", this.currentText);
                this.currentPosition = 0;
                if (this.sessionIdEl) {
                ...
                this.stats.errors = 0;
                this.stats.score = data.total_score || 0;
    ...
    async requestNewText() {
        try {
            ...
            if (response.ok) {
                const data = await response.json();
                this.currentText = data.text;
                console.log("FRONTEND TEXT:", this.currentText);
                this.currentPosition = 0;
                this.progressFill.style.width = '0%';
                this.lastKeystrokeTime = null;
                this.renderText();
