...
def start_session():
    ...
    # FIX #1: Block start_session without login
    if not user_id:
...
def login():
    ...
    print("🔥 LOGIN LEVEL:", progress.current_level if progress else "None")
    return jsonify({
        'status': 'ok',
        'user_id': user_id,
...
def save_progress():
    ...
    # Fix: Normalize user_id to int to match session storage
    try:
        user_id = int(user_id)
    except (ValueError, TypeError):
        return jsonify({'error': 'Invalid user_id'}), 400
...
def process_keystroke():
    ...
    if result.get('error') == 'Session not found':
        game_engine.start_session(session_id)
        result = game_engine.process_keystroke(session_id, key, timestamp)
...
@app.route('/api/new_text/<session_id>')
